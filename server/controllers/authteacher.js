...
export const register = async (req, res) => {
  ...
  try {
    ...
    if (!userteacher) {
      return res
        .status(400)
        .send({ message: "you are not a faculty ID incorrect" });
    }
...
export const login = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ message: "Login succesfully", userRole: "faculty", user });
  } catch (error) {
...
export const detailsById = async (req,res) => {
  ...
  try {
    ...
    user = await teachermodel.findOne({ teacher_id });
    if (!user) {
...
export const deleteById = async (req,res) => {
  ...
  try {
    ...
    let user=await teachermodel.findOne({teacher_id:ID})
    user = await teachermodel.findByIdAndDelete(user._id);
