...
export const create = async (req, res) => {
  const { subject, start_year, branch, USER_ID } = req.body;
    console.log(subject, start_year, branch, USER_ID )
  if (!subject || !start_year || !branch || !USER_ID) {
    return res.status(400).send({ message: "class details not found" });
  }
  try {
    const year = await studentmodel.findOne({ start_year, branch:branch.toUpperCase() });
    if (!year) {
    ...
    if (user) {
      return res.status(400).send({ message: "class is already exist" });
    }
    const classroom = {
      subject:subject.toUpperCase(),
      start_year,
...
export const gather = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(404)
      .send({ message: "error in gather teacher object data" });
  }
};
...
export const Update = async (req, res) => {
  try {
    ...
    const user = await classmodel.findByIdAndUpdate(id, {
      subject:subject.toUpperCase(),
      start_year,
      branch:branch.toUpperCase(),
    });
