  ...
  useEffect(() => {
    const userdetails = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:8080/api/authstudent/stduentdetails", {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-100 flex items-center justify-center px-4">
      <div className="bg-white max-w-4xl w-full rounded-2xl shadow-2xl p-10 flex flex-col md:flex-row items-center md:items-start gap-10 md:gap-20">
        ...
        <div className="flex-1">
          <h2 className="text-3xl font-bold text-green-800 mb-6 border-b pb-2">Student Profile</h2>
          <div className="space-y-4 text-lg text-gray-700">
            <p><span className="font-semibold text-gray-900">Name:</span> {user.name}</p>
            <p><span className="font-semibold text-gray-900">Roll No:</span> {user.rollNo}</p>
            ...
            <p><span className="font-semibold text-gray-900">Branch:</span> {user.branch}</p>
            <p><span className="font-semibold text-gray-900">Start Year:</span> {user.start_year}</p>
          </div>
        </div>
      </div>
    </div>
